...
{utp/ut-api-action.i piMetaDataList GET /metadata/list/~* }
{utp/ut-api-action.i piMetaDataForm GET /metadata/form/~* }
...
/* Vari†veis com informaá‰es da lista carregada pela pesquisa */
...
PROCEDURE piMetaDataList:
    DEFINE INPUT  PARAMETER jsonInput  AS JsonObject NO-UNDO.
    DEFINE OUTPUT PARAMETER jsonOutput AS JsonObject NO-UNDO.

    DEFINE VARIABLE aFields AS JsonArray NO-UNDO.
    DEFINE VARIABLE oField  AS JsonObject NO-UNDO.
    DEFINE VARIABLE oAction AS JsonObject NO-UNDO.
    
    jsonOutput = NEW JSONObject().
    aFields    = NEW JsonArray().
    oField     = NEW JSONObject().
    oAction    = NEW JSONObject().
    
    jsonOutput:ADD("version", 1).
    jsonOutput:ADD("title", "Clientes").
    
    oField = NEW JsonObject().
    oField:ADD("property", "Cust-Num").
    oField:ADD("label", "N£mero").
    oField:ADD("key", TRUE).
    aFields:ADD(oField).

    oField = NEW JsonObject().
    oField:ADD("property", "Name").
    oField:ADD("label", "Nome").
    aFields:ADD(oField).  
    
    oField = NEW JsonObject().
    oField:ADD("property", "Sales-Rep").
    oField:ADD("label", "Representante").
    aFields:ADD(oField).   
 
    oField = NEW JsonObject().
    oField:ADD("property", "Contact").
    oField:ADD("label", "Contato").
    aFields:ADD(oField).  
    
    oField = NEW JsonObject().
    oField:ADD("property", "Credit-Limit").
    oField:ADD("label", "Limite de crÇdito").
    aFields:ADD(oField).      
    
    oAction = NEW JsonObject().
    oAction:ADD("new", "/new").
    oAction:ADD("edit", "edit/:id").
    oAction:ADD("remove", TRUE).
    
    jsonOutput:ADD("fields", aFields).
    jsonOutput:ADD("actions", oAction).
    
END PROCEDURE.

PROCEDURE piMetaDataForm:
    DEFINE INPUT  PARAMETER jsonInput  AS JsonObject NO-UNDO.
    DEFINE OUTPUT PARAMETER jsonOutput AS JsonObject NO-UNDO.
    ...
    oField:ADD("label", "N£mero").
    oField:ADD("key", TRUE).
    oField:ADD("divider", "Dados pessoais").
    oField:ADD("disabled", TRUE).
    oField:ADD("filter", TRUE).
    oField:ADD("gridMdColumns", 4).
    aFields:ADD(oField).
    ...
    oField:ADD("label", "Nome").
    oField:ADD("filter", TRUE).
    oField:ADD("gridMdColumns", 8).
    oField:ADD("required", TRUE).
    aFields:ADD(oField).  
    
    oField = NEW JsonObject().
    oField:ADD("property", "Phone").
    oField:ADD("label", "Telefone").
    oField:ADD("required", TRUE).
    oField:ADD("gridMdColumns", 4).
    aFields:ADD(oField).   
    ...
    oField = NEW JsonObject().
    oField:ADD("property", "country").
    oField:ADD("divider", "Endereáo").
    oField:ADD("label", "Pa°s").
    oField:add("gridMdColumns", 3).
    aFields:ADD(oField).  
    
    oField = NEW JsonObject().
    oField:ADD("property", "State").
    oField:ADD("label", "Estado").
    oField:ADD("gridMdColumns", 3).
    aFields:ADD(oField).  

    oField = NEW JsonObject().
    oField:ADD("property", "City").
    oField:ADD("label", "Cidade").
    oField:ADD("gridMdColumns", 3).
    aFields:ADD(oField).  

    oField = NEW JsonObject().
    oField:ADD("property", "Postal-Code").
    oField:ADD("label", "C¢digo postal").
    oField:ADD("gridMdColumns", 3).
    oField:ADD("required", TRUE).
    aFields:ADD(oField).  

    oField = NEW JsonObject().
    oField:ADD("property", "Address").
    oField:ADD("label", "Endereáo").
    oField:ADD("gridMdColumns", 12).
    oField:ADD("required", TRUE).
    aFields:ADD(oField). 

    oField = NEW JsonObject().
    oField:ADD("property", "Sales-Rep").
    oField:ADD("divider", "Comercial").
    oField:ADD("label", "Representante de vendas").
    oField:add("gridMdColumns", 3).
    aFields:ADD(oField).  
    ...
    oField:ADD("property", "Credit-Limit").
    oField:ADD("label", "Limite de crÇdito").
    oField:ADD("type", "number").
    oField:add("gridMdColumns", 3).
    aFields:ADD(oField).  

    oField = NEW JsonObject().
    oField:ADD("property", "Balance").
    oField:ADD("label", "Saldo").
    oField:ADD("type", "number").
    oField:add("gridMdColumns", 3).
    aFields:ADD(oField).  

    oField = NEW JsonObject().
    oField:ADD("property", "Discount").
    oField:ADD("label", "Desconto").
    oField:ADD("type", "number").
    oField:add("gridMdColumns", 3).
    aFields:ADD(oField).     
...
PROCEDURE piGet:
    ...
    IF NOT qCustomer:QUERY-OPEN() THEN
        ASSIGN jsonOutput = JsonAPIResponseBuilder:asError(NEW Progress.Lang.AppError('Erro na pesquisa de Customer. Verifique os parametros da requisi‚Ä°√Üo',4), 400).
...
PROCEDURE piPut:
    ...
    END.
    ELSE
        ASSIGN jsonOutput = JsonAPIResponseBuilder:asError(NEW Progress.Lang.AppError('Erro na altera‚Ä°√Üo de Customer',2), 400).
